...
impl<'a, Tree: 'static + MerkleTreeTrait, G: 'static + Hasher> StackedDrg<'a, Tree, G> {
        ...
        measure_op(Operation::GenerateTreeC, || {
            ...
            rayon::scope(|s| {
                ...
                s.spawn(move |_| {
                    ...
                    let column_batcher =
                        Batcher::<ColumnArity>::pick_gpu(max_gpu_column_batch_size)
                            .expect("failed to create GPU batch column hasher");
                    let tree_batcher = Batcher::<TreeArity>::pick_gpu(max_gpu_tree_batch_size)
                        .expect("failed to create GPU batch tree hasher");
        ...
        TreeArity: PoseidonArity,
    {
        ...
        rayon::scope(|s| {
            ...
            s.spawn(move |_| {
                let _gpu_lock = GPU_LOCK.lock().expect("failed to get gpu lock");
                let tree_batcher = Batcher::<Tree::Arity>::pick_gpu(max_gpu_tree_batch_size)
                    .expect("failed to create GPU batch tree hasher");
        ...
        TreeArity: PoseidonArity,
    {
        ...
        if SETTINGS.use_gpu_tree_builder {
            ...
            let tree_batcher = Batcher::<TreeArity>::pick_gpu(max_gpu_tree_batch_size)
                .expect("failed to create GPU batch tree hasher");
            let mut tree_builder = TreeBuilder::<TreeArity>::new(
                #[cfg(feature = "gpu")]
                Some(tree_batcher),
                nodes_count,
                tree_r_last_config.rows_to_discard,
